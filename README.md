...
- **Trained model artifacts:** [`models/roberta-trained-welfake/`](models/roberta-trained-welfake/)
...
2. verify every input in parallel with web-evidence retrieval and natural-language inference (NLI),
   presented as supporting context only;
...
WELFake test split and selects the 100 lowest-confidence articles for an **offline escalation
experiment**. This is not a production confidence gate: the deployed pipeline runs retrieval and
NLI verification for every input, presents the verdict as supporting context, and keeps the
classifier label final.

For the offline experiment, each selected article's title is passed to `verify.py`. If the title is
empty, the first 300 characters of the combined content are used instead.
...
The offline escalation experiment uses this counterfactual mapping:
...
and the NLI verdict distribution for the low-confidence bucket. The committed CSV preserves the
completed article-level evaluation used by the report.
...
See the [results summary](docs/results-summary.md) for the full classifier, uncertainty,
calibration, escalation, and explanation-faithfulness results. The completed single-reviewer
faithfulness study is committed in `evaluation/faithfulness_review.csv`: **8 of 10 explanations
