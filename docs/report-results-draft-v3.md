...
**The classifier reached near-ceiling accuracy within WELFake \cite{verma2021welfake}, but we do not treat that score as cross-dataset evidence.** We evaluated RoBERTa-LoRA on the deterministic, stratified 70/15/15 split with seed 42 rather than drawing a fresh random split, preserving class balance and reproducibility. The held-out test set contained 9,398 unseen articles, comprising 5,193 real and 4,205 fake articles. We report accuracy, per-class precision, recall and F1, with unweighted macro averages. Table 1 gives the scores that the error analysis then explains.
