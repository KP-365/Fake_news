...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[
    ...
    block/.style={draw, rounded corners, align=center, minimum height=8mm,
                  minimum width=23mm, font=\small},
    signal/.style={draw, rounded corners, align=center, minimum height=7mm,
                   minimum width=20mm, font=\small},
    every edge/.style={draw, -{Latex}, thick}
  ]
    \node[block] (article) at (0,0) {Article};
    \node[block] (classifier) at (3.0,1.4) {RoBERTa--LoRA\\classifier};
    \node[signal] (label) at (6.1,2.0) {Deterministic\\label};
    \node[block] (mc) at (6.1,0.8) {MC Dropout\\side branch\\30 passes};
    \node[signal] (stability) at (8.8,0.8) {Stability\\signal};
    \node[block] (retrieval) at (3.0,-1.4) {DDG retrieval\\top-$5$ snippets};
    \node[block] (nli) at (6.1,-1.4) {DeBERTa NLI};
    \node[signal] (verdict) at (8.8,-1.4) {Verdict};
    \node[block] (explanation) at (12.0,0) {Explanation\\agent};

    \path (article) edge (classifier);
    \path (article) edge (retrieval);
    \path (classifier) edge (label);
    \path (classifier) edge (mc);
    \path (mc) edge (stability);
    \path (retrieval) edge (nli);
    \path (nli) edge (verdict);
    \path (label) edge (explanation);
    \path (stability) edge (explanation);
    \path (verdict) edge node[midway, above, align=center, fill=white,
      inner sep=1pt, font=\scriptsize] {supporting\\context only} (explanation);
  \end{tikzpicture}
